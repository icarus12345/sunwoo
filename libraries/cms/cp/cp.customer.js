var customer_config = {
    ...
    'entryType':'',
    'entryId': '',
    'entryBindingUri': base_url+'cms/cp/customer/bindingbytype/',
    'entryEditUri': base_url+'cms/cp/customer/editpanel/',
    'entryCommitUri': base_url+'cms/cp/customer/oncommit/',
    'entryDeleteUri': base_url+'cms/cp/customer/ondelete/'
};
    ...
    return {
        ...
        'createtable' :   function(){
            oTable = $('#entryDatatable').dataTable({
                ...
                'bServerSide': true,
                'bPaginate': true,
                'sAjaxSource': customer_config.entryBindingUri+customer_config.entryType,
                'bJQueryUI': false,
                'bAutoWidth': false,
        ...
        'onEditItem': function(Id){
            ...
            $('#iEntryDialog').html('...');
            httpRequest({
                'url'         :   customer_config.entryEditUri + customer_config.entryType,
                'data'        :   {
                    'Id'  :   Id
                },
                'callback'    :   function(rsdata){
        ...
        'onSave': function(){
            ...
            this.onCommit(customer_config.entryCommitUri,Params,Id,function(rsdata){
                if(rsdata.result<0){
                    addNotice(rsdata.message,'danger');
                }else{
                    customer.onRefresh();
        ...
        'onDeleteItem': function(Id){
            ConfirmMsg(
                ...
                function(){
                    httpRequest({
                        'url': customer_config.entryDeleteUri,
                        'data': {
                            'Id': Id
                        },
                        'callback': function(rsdata) {
        ...
        'changeStatus':function(status,Id){
            this.onCommit( customer_config.entryCommitUri, {
                'Status':status
            }, Id, function(rsdata){
                if(rsdata.result>=0){
                    customer.onRefresh();
        ...
        'onChangeSource': function(type){
            if(type!==customer_config.entryType){
                customer_config.entryType=type;
                oTable.fncustomers().sAjaxSource = (customer_config.entryBindingUri+customer_config.entryType);
                oTable.fnDraw(false);
            }
        },
        'toggleElm': function(elm){
