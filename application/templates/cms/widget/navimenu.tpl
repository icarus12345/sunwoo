<ul>
    <li class="menu-header"><a href="#"> CONTENT PROVIDER</a></li>
            <li data-id="136">
        <a href="/cms"><i class="fa fa-question"></i>Help</a>
            </li>
        <li data-id="150">
        <a href="/cms/cp/data/vp/.a.e./service"><i class="fa fa-folder"></i>Service</a>
            </li>
        <li data-id="141">
        <a href="/cms/cp/news/vp/.e./about"><i class="fa fa-info-circle"></i>Abouts</a>
            </li>
        <li data-id="129">
        <a href="#"><i class="fa fa-newspaper-o"></i>Blogs</a>
            <ul class="submenu">
                            <li><a href="/cms/cp/cate/vp/.a.e./default?navi=129"><i class="fa fa-bars"></i>Categories</a></li>
                                    <li><a href="/cms/cp/news/vp/.a.d.e./default?navi=129"><i class="fa fa-newspaper-o"></i>All Blogs</a></li>
                        </ul>
            </li>
        <li data-id="138">
        <a href="#"><i class="fa fa-bars"></i>Shop</a>
            <ul class="submenu">
                            <li><a href="/cms/cp/supplier/vp/.a.e.d./default?navi=138"><i class="fa fa-tags"></i>Supplier</a></li>
                                    <li><a href="/cms/cp/cate/vp/.a.e.d./default?navi=138"><i class="fa fa-tree"></i>Categories</a></li>
                                    <li><a href="/cms/cp/line/vp/.a.d.e./default?navi=138"><i class="fa fa-cogs"></i>Setting Color|Size|Materia</a></li>
                                    <li><a href="/cms/cp/product/vp/.a.d.e./default?navi=138"><i class="fa fa-newspaper-o"></i>Product</a></li>
                        </ul>
            </li>
        <li data-id="135">
        <a href="/cms/cp/lang/vp/.a.e./default?#"><i class="fa fa-list"></i>Translate</a>
            </li>
        <li data-id="3">
        </li>
            <li class="menu-header"><a href="#"> SERVICE OPERATOR</a></li>
            <li data-id="5">
        <a href="#"><i class="fa fa-cogs"></i>Settings</a>
            <ul class="submenu">
                            <li><a href="/cms/cp/setting/vpe/.a.d.e./default?navi=5"><i class="fa fa-folder"></i>Other</a></li>
                                    <li><a href="/cms/fm/file/">KCFinder - Image manage</a></li>
                        </ul>
            </li>
        <li data-id="6">
        <a href="/cms/account"><i class="fa fa-group"></i>User Account</a>
            </li>
        </ul>